...
- `current_state` CHECK на ровно 8 значений: `CREATED, ACCEPTED_BY_UK, SENT_TO_CONTRACTOR, EXECUTION,
...
- closure projection CHECK (§ 24 п. 8): `current_state='COMPLETED' ⇒ closed_at, closed_by_user_id, closure_kind
  NOT NULL`; `current_state<>'COMPLETED' ⇒ closed_at/closed_by_user_id/closure_kind/closure_explanation NULL`;
  `closure_kind` из `CONFIRMED_RESULT | NO_RESIDENT_FEEDBACK | DISPUTED_WITH_EXPLANATION`.
- `revision >= 1`, `last_event_seq >= 0`; partial unique `uq_case_display_number` на `display_number`
  (nullable).
- `demo_run_id` → `DemoRun` (single FK, nullable).
...
- `UNIQUE(case_id, selection_no)`; **без** `is_current`-флага — current определяется только
  `Case.current_selection_id`.
...
- полностью immutable; `UNIQUE(iteration_id)` (один Result на iteration), `UNIQUE(case_id, result_id)` для
  same-case composite FK.
...
- `caused_by_event_id` self-FK nullable; EVT-014 derived/caused-by-связь — семантика domain layer
  (TG-009/TG-016), не DB-триггер.

### 7.9. DemoRun deferred FK (Data Model § 26.6)

- в `0002_case_workflow`: `Case.demo_run_id` single FK → `DemoRun`; partial unique
  `UNIQUE(demo_run_id, case_id) WHERE demo_run_id IS NOT NULL`; composite
  `DemoRun(demo_run_id, primary_case_id) → Case(demo_run_id, case_id)` (declared deferred в TG-005 § 7).
...
- current pointers и каждый child c `iteration_id`/`assignment_id`/`result_id` используют composite
  `(case_id, <id>)`; cross-Case ссылки физически невозможны (negative fixtures § 10).
...
Каждый констант/индекс объявляется отдельным оператором с приведённым именем (прецедент TG-005 § 7). Примечание:
имена ниже нормативны для тестов каталога; точные column-наборы берутся из §§ 9–18, 25.

```text
ck_case_current_state                 CHECK Case.current_state IN (8 states)
ck_case_closure_consistency           CHECK closure projection по § 24 п.8
ck_case_revision                      CHECK revision >= 1
ck_case_last_event_seq                CHECK last_event_seq >= 0
uq_case_display_number                UNIQUE(display_number) WHERE display_number IS NOT NULL
...
fk_case_org_id          / fk_case_house_id / fk_case_premises_id / fk_case_resident_user_id /
fk_case_category_id     / fk_case_created_by_user_id / fk_case_demo_run_id /
...
fk_iteration_case_id / fk_iteration_started_by_user_id / fk_iteration_source_result_id /
fk_iteration_source_feedback_id / fk_iteration_started_by_event_id
cq_selection_case_selection           UNIQUE(case_id, selection_id)
cq_selection_case_no                  UNIQUE(case_id, selection_no)
fk_selection_case_id / fk_selection_created_iteration_id / fk_selection_contractor_id /
...
fk_assignment_case_id / fk_assignment_selection_id / fk_assignment_contractor_id /
fk_assignment_created_iteration_id / fk_assignment_sent_by_user_id / fk_assignment_accepted_by_user_id /
...
fk_result_case_id / fk_result_iteration_id / fk_result_assignment_id / fk_result_contractor_id /
fk_result_author_user_id
uq_feedback_result_id                 UNIQUE(result_id)
ck_feedback_type                      CHECK type IN ('CONFIRMATION','REMARK')
ck_feedback_remark                    CHECK (type='REMARK' AND remark_text непуст) OR (type='CONFIRMATION' AND remark_text NULL/пуст)
fk_feedback_case_id / fk_feedback_iteration_id / fk_feedback_result_id / fk_feedback_resident_user_id
ck_comment_kind                       CHECK comment_kind IN ('WORKING','CLARIFICATION_REQUEST','CLARIFICATION_REPLY')
fk_comment_case_id / fk_comment_iteration_id / fk_comment_author_user_id /
fk_comment_context_result_id / fk_comment_context_feedback_id / fk_comment_in_reply_to_comment_id
uq_event_case_seq                     UNIQUE(case_id, event_seq)
ck_event_type                         CHECK event_type IN ('EVT_001'..'EVT_017')
ck_event_evt015_result                CHECK (event_type <> 'EVT_015' OR result_id IS NOT NULL)
uq_evt015_per_result                  UNIQUE(result_id, event_type) WHERE event_type = 'EVT_015'
fk_event_case_id / fk_event_iteration_id / fk_event_selection_id / fk_event_assignment_id /
fk_event_result_id / fk_event_feedback_id / fk_event_comment_id / fk_event_caused_by_event_id /
fk_event_actor_user_id
fk_demo_run_primary_case              DemoRun(demo_run_id, primary_case_id) → Case(demo_run_id, case_id)
uq_case_demo_run_case                 UNIQUE(demo_run_id, case_id) WHERE demo_run_id IS NOT NULL
```
...
11. Frozen workspace checks green: `npm run typecheck`, `npm run build`, workspace `test` script (`vitest run
    src/index.test.ts`) и TG-005 integration suite не регрессируют (правки № 3–6 § 5 — только расширение каталога).
...
Real PostgreSQL (Postgres 17/18); SQLite/in-memory/mock — не evidence, запрещено. Соединение: env
`TG006_TEST_DATABASE_URL`, DB-name guard `current_database()` оканчивается на `_tg006_test`. Тесты изолированы
...
$env:TG006_TEST_DATABASE_URL = '<url: db name *_tg006_test>'
npx vitest run packages/db/src/db-foundation.integration.test.ts     # обновлённый каталог TG-006
npx vitest run packages/db/src/case-workflow.integration.test.ts
```
...
1. **clean migrate:** в dedicated schema (например `tg006_case_workflow`) migrateToLatest из source; ровно
   21 app-таблица; exact-catalog проверка registry § 7.13 (имена + колонки + предикаты); `rollbackAll` → 0
...
3. **cross-Case negative matrix:** Result/Feedback/Comment/Event/Selection/Assignment с `case_id` родителя,
   отличным от case их child-parent → 23503; current pointers другого Case → 23503.
4. **uniqueness:** второй Result на iteration → 23505; второй feedback на Result → 23505; duplicate
   `(case_id,event_seq)` → 23505; duplicate `(case_id,iteration_no)`, `(case_id,selection_no)`,
   `(case_id,assignment_no)` → 23505; second Assignment на тот же Selection → 23505.
5. **EVT-015:** `EVT_015` c NULL result → 23514; второй `EVT_015` на тот же result → 23505.
6. **immutability UPDATE/DELETE matrix:** для Result, ResidentFeedback, Comment, CaseEvent, CaseIteration
   identity, ContractorSelection, Assignment identity плюс одна-way decision флагов (allowed сценарий positive,
...
7. **event sequencing:** вставка события с нарушенной `event_seq`-уникальностью rejected; ок исходящая
   sequences проходят; выборка упорядочена.
8. **same-contractor N+1 Result fixture:** bootstrap Case (iteration 1) → Assignment accepted в iteration 1 →
...
9. **demo_run FK:** `primary_case_id` другого `demo_run_id` → 23503; valid same-run → ok.
10. **compiled-mode smoke** (по прецеденту TG-005 § 13.2): после `npm run build` `dist/migrations/0002_case_workflow.js`
    загружается `FileMigrationProvider` и регистрируется в миграционной цепочке.

Offline `case-workflow.unit.test.ts` — DDL-манифест: 8 новых CREATE TABLE, exact имена partial-индексов
(`uq_evt015_per_result`, `uq_case_display_number`, `uq_case_demo_run_case`) и их предикаты, отсутствие
state-machine-переходов в trigger-текстах (только immutability-защита).
