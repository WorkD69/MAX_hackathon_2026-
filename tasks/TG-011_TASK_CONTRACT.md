...
- **Idempotency replay не обходит authorization.** На каждом replay request сначала authenticate application session, заново resolve/revalidate current authoritative binding/context и применить resource visibility/authorization; только после успешной проверки relevant Case/resource/context TG-012 может вернуть сохранённый successful response по прежнему idempotency key. Отозванный binding/access или утративший current Assignment/executor после reassignment подрядчик не получает protected replay response даже при действующем token и существующей successful replay entry. Для hidden resource replay не раскрывает ни существование сохранённого success/Case, ни internal state; применяется тот же hidden-404 ordering, что и при обычном request. Этот gate проверяет актуальное право видеть relevant resource/context, а не повторяет business transition исходной команды: всё ещё авторизованный principal сохраняет legitimate replay по TG-012 semantics. TG-011 задаёт security gate, а `CommandExecution` reservation/replay и его допустимый ответ остаются реализацией TG-012; внутренний порядок reservation здесь не предписывается.
...
Policy на каждом request, включая idempotency replay, выдаёт decision и применимые scope/visibility descriptors без stale rights; отозванный binding/access блокирует следующий request при live token до выдачи stored success. Ровно четыре Product roles (`RESIDENT`, `UK_EMPLOYEE`, `UK_ADMIN`, `CONTRACTOR_EMPLOYEE`); pending/current executor — **policy states**, не роли. Resident/UK/admin tenant границы, три contractor стадии, немедленная потеря LIVE прав old contractor и hidden-404 ordering соблюдены также для replay. TG-011 не мутирует Case/domain facts; consumers могут применить policy до projection/command validation и возврата replayed response.
...
- Replay после binding revocation: начальный authorized command создаёт successful replay entry, binding отозван, следующий request с тем же idempotency key и live token denied; stored response не выдаётся.
- Replay после contractor reassignment: ранее accepted old contractor имеет successful entry, Case переназначен, повтор с тем же key denied без protected response.
- Hidden-resource replay: unauthorized request не раскрывает существование stored success/Case или internal state и сохраняет hidden-404 ordering независимо от наличия replay entry.
- Authorized unchanged principal: текущая authorization проходит и legitimate replay остаётся доступным по downstream TG-012 idempotency semantics.
